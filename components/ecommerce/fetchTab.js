...
import producttype from  '../../jsondata/producttype/producttype.json'
function FeatchTab() {
  ...
  const fillterProduct = async () => {
    try {
      ...
      if (producttype && productType === "") {
        const firstThreeItems = producttype?.slice(0, 4);

        setNewProduct(firstThreeItems);
        setActive("0");
      }
      if (producttype && productType === "0") {
        setNewProduct(producttype);
        setActive("0");
      }
...
const [ptypedata,setproducttypedata]=useState()
  useEffect(()=>{
    const productype=producttype.filter((ptype)=>ptype?.productType==productType)
    setproducttypedata(productype)
      ...
      <div className="tab-content wow fadeIn animated">
        ...
        >
          <div className="product-grid-4 row">
            <FeaturedTab products={ptypedata} />
          </div>
        </div>
