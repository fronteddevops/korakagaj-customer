...
import producttypemore from "../../jsondata/producttype/producttype.json";
...
const Products = ({ products1, productFilters }) => {
  ...
  const producttpe = new URLSearchParams(window.location.search).get("product");
  const [sortOrder, setSortOrder] = useState("Default");

  useEffect(() => {
    const filteredProducts = producttypemore.filter((product) => {
      const priceInRange =
        product.finalAmount >= price?.value?.min &&
        product?.finalAmount <= price?.value?.max;
  
      const hasDesiredSize = selectedSizes.length === 0 || 
        JSON.parse(product.size).some((size) =>
          selectedSizes.includes(size)
        );
  
      return priceInRange && hasDesiredSize;
    });
    if (sortOrder === "LowToHigh") {
      filteredProducts.sort((a, b) => a.finalAmount - b.finalAmount);
    } else if (sortOrder === "HighToLow") {
      filteredProducts.sort((a, b) => b.finalAmount - a.finalAmount);
    }
    setProdcut(filteredProducts);
  }, [price, selectedSizes,sortOrder]);
  ...
  useEffect(() => {
    if (producttpe) {
      const products = producttypemore.filter(
        (data) => data?.productType == producttpe
      );
      setProdcut(products);
    } else {
      setProdcut(producttypemore);
    }
  }, [producttpe]);
